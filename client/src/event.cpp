...
Event::Event(std::string team_a_name, std::string team_b_name, std::string name, int time,
    ...
    : team_a_name(team_a_name), team_b_name(team_b_name), name(name),
      ...
      team_b_updates(team_b_updates), description(discription),beforeHalftime("true")
{
}
...
const std::string &Event::gethalftime()const{
    return this->beforeHalftime;
}
std::string &Event::sethalftime(std::string value)
{
    beforeHalftime=value;
}
